  ...
  return (
    <footer className={`pt-12 pb-12 transition-colors duration-500 border-t border-black/10`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* 4-Column Layout */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 md:gap-12 pb-16">
          ...
          <div className="lg:col-span- space-y-4">
            ...
            <ul className="space-y-2.5 text-sm font-medium">
              {[
                {
                  label: locale === 'de' ? 'Gartenpakete' : 'Garden Packages',
                  href: `/${locale}/gardens`,
                },
                {
                ...
                {
                  label: locale === 'de' ? 'Pflanzanleitung' : 'Planting Guide',
                  href: `/${locale}/plant-guide`,
                },
              ].map((link, i) => (
                <li key={i}>
                  <a
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-3 text-xs font-medium text-neutral-600">
            <span>
              © {new Date().getFullYear()} eledesign.{' '}
              {locale === 'de' ? 'Alle Rechte vorbehalten.' : 'All rights reserved.'}
            </span>

            <span className="opacity-50">•</span>

            <Link href={`/${locale}/privacy`} className="hover:text-neutral-900 transition-colors">
          ...
          <button
            onClick={scrollToTop}
            className={`flex items-center gap-2 text-xs font-medium uppercase tracking-wider cursor-pointer group select-none `}
          >
            <span>{locale === 'de' ? 'Zurück nach oben' : 'Back to top'}</span>
            <div
              className={`p-1.5 rounded-full border-2 border-stone-600 transition-all duration-300 group-hover:-translate-y-0.5  bg-white `}
            >
